...
@section('content')
    <div class="mb-6 flex items-center justify-between gap-4">
        <a href="{{ route('dashboard') }}" class="brand-button-soft inline-flex items-center rounded-full border border-slate-200 bg-white/70 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:text-slate-950">
            {{ __('ui.back_to_dashboard') }}
        </a>

        <a href="{{ route('products.show', $product) }}" class="brand-button inline-flex items-center rounded-full bg-slate-950 px-4 py-2 text-sm font-semibold text-white transition hover:bg-slate-800">
            {{ __('ui.view_product') }}
        </a>
    </div>
    ...
    <section class="glass-panel rounded-[32px] p-6 sm:p-8">
        ...
        <form method="POST" action="{{ route('dashboard.products.update', $product) }}" enctype="multipart/form-data" class="grid gap-5">
            ...
            <div class="grid gap-5 lg:grid-cols-3">
                <div>
                    ...
                    <input id="title" name="title" value="{{ old('title', $product->title) }}" required class="brand-field w-full rounded-2xl border border-slate-200 bg-white/80 px-4 py-3 text-sm outline-none transition focus:border-amber-400 focus:ring-4 focus:ring-amber-200/70">
                ...
                <div>
                    ...
                    <input id="position" name="position" type="number" min="0" value="{{ old('position', $product->position) }}" required class="brand-field w-full rounded-2xl border border-slate-200 bg-white/80 px-4 py-3 text-sm outline-none transition focus:border-amber-400 focus:ring-4 focus:ring-amber-200/70">
                ...
                <div>
                    ...
                    <input id="images" name="images[]" type="file" multiple accept="image/*" class="brand-field w-full rounded-2xl border border-slate-200 bg-white/80 px-4 py-3 text-sm outline-none transition file:mr-4 file:rounded-full file:border-0 file:bg-[var(--brand-ink)] file:px-4 file:py-2 file:text-sm file:font-semibold file:text-white focus:border-amber-400 focus:ring-4 focus:ring-amber-200/70">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="6" required class="brand-field w-full rounded-[24px] border border-slate-200 bg-white/80 px-4 py-3 text-sm outline-none transition focus:border-amber-400 focus:ring-4 focus:ring-amber-200/70">{{ old('description', $product->description) }}</textarea>
        ...
        <form method="POST" action="{{ route('dashboard.products.destroy', $product) }}" onsubmit="return confirm('{{ __('ui.confirm_delete_product') }}')" class="mt-4">
            @csrf
            @method('DELETE')
            <button type="submit" class="brand-button-soft rounded-2xl border border-red-200 px-5 py-3 text-sm font-semibold text-red-600 transition hover:border-red-300 hover:text-red-700">
